...
import {
  Activity, Heart, Brain, CheckCircle, AlertTriangle,
  ArrowRight, Stethoscope, Info, CheckCircle2, AlertCircle,
  User, ClipboardList, Dumbbell, ChevronRight, Zap
} from 'lucide-react';
...
/* --- PAGE 2: UPDATED PREDICTION FORM --- */
const PredictionForm = () => {
  const [probability, setProbability] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
...
const response = await fetch("https://ml-cardio.onrender.com/predict", {
          method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ data: values }),
      });
  ...
  const getRiskDetails = (prob) => {
    if (prob > 0.7) return { label: 'High Risk', color: '#EF4444', bg: '#FEF2F2' };
    if (prob > 0.4) return { label: 'Moderate Risk', color: '#F59E0B', bg: '#FFFBEB' };
    return { label: 'Low Risk', color: '#10B981', bg: '#ECFDF5' };
  };
              ...
              <div style={{ width: '100%', animation: 'fadeIn 0.5s ease' }}>
                <div style={{
                  ...
                  <div style={{ marginBottom: '12px', display: 'inline-block' }}>
                    {prediction === 1 ? <AlertTriangle size={50} /> : <CheckCircle size={50} />}
                  </div>
                  <h2 style={{ fontSize: '1.6rem', fontWeight: '800', margin: '0 0 8px 0' }}>
                    {prediction === 1 ? 'Condition Detected' : 'Condition Not Detected'}
                  </h2>
                  <p style={{ margin: 0, fontWeight: '500', opacity: 0.9, fontSize: '0.9rem' }}>
                    {prediction === 1 ? "Cardiovascular patterns identified." : "No significant risk patterns identified."}
                  </p>
                </div>

                {/* NEW PROBABILITY SCORE SECTION */}
                {probability !== null && (
                  <div style={{ background: 'white', padding: '20px', borderRadius: '16px', border: '1px solid #E2E8F0', marginBottom: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span style={{ fontWeight: '700', fontSize: '0.9rem', color: '#475569' }}>PROBABILITY SCORE</span>
                      </div>
                      <span style={{ 
                        fontSize: '0.75rem', 
                        fontWeight: '800', 
                        ...
                        backgroundColor: getRiskDetails(probability).bg,
                        color: getRiskDetails(probability).color
                      }}>
                        {getRiskDetails(probability).label}
                      </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                      <div style={{ flex: 1, height: '10px', background: '#F1F5F9', borderRadius: '10px', overflow: 'hidden' }}>
                        <div style={{ 
                          height: '100%', 
                          background: getRiskDetails(probability).color, 
                          width: `${(probability * 100).toFixed(2)}%`,
                          transition: 'width 1.5s cubic-bezier(0.4, 0, 0.2, 1)',
                          borderRadius: '10px'
                        }}></div>
                      </div>
                      <span style={{ fontSize: '1.2rem', fontWeight: '800', color: '#0F172A', minWidth: '60px' }}>
